...
from deformable_LKA import deformableAttention
...
class LMSCNet_SS(MVXTwoStageDetector):
    def __init__(self,
        ...
        self.atten_68 = deformableAttention(68)
        self.atten_60 = deformableAttention(60)

        self.feature_cat_out1_4 = BFFM(in_channels_x=4, in_channels_y=64, mid_channels=4)
        self.feature_cat_out1_2_1 = BFFM(in_channels_x=8, in_channels_y=48, mid_channels=8)
        self.feature_cat_out1_2_2 = BFFM(in_channels_x=56, in_channels_y=4, mid_channels=56)
        ...
        if self.out_scale=="1_1":
          ...
          #self.aspp_senet = ASPP_SENet(inplanes=1, planes=8, dilations_conv_list=[1, 2, 3])
    ...
    def step(self, input):
        ...
        #特征融合
          out = self.feature_cat_out1_4(out, _skip_1_4)

          out = self.atten_68(out)
          ...
          out1= self.deconv_1_8__1_2(out_scale_1_8__2D)

          #out = torch.cat((out, _skip_1_2, out1), 1)

          #特征融合
          out = self.feature_cat_out1_2_1(out, _skip_1_2)
          out = self.feature_cat_out1_2_2(out,out1)

          out = self.atten_60(out)

          out = F.relu(self.conv1_2(out)) # torch.Size([1, 48, 128, 128])
          out_scale_1_2__2D = self.conv_out_scale_1_2(out) # torch.Size([1, 16, 128, 128])

          out_scale_1_2__3D = self.seg_head_1_2(out_scale_1_2__2D) # [1, 2, 16, 128, 128]
    ...
    def foward_test(self,
        ...
        # msnet3d
        if not os.path.exists(os.path.join("/media/s/新加卷/voxkitti/dataset/sequences_msnet3d", img_metas[0]['sequence_id'], 'queries')):
            os.makedirs(os.path.join("/media/s/新加卷/voxkitti/dataset/sequences_msnet3d", img_metas[0]['sequence_id'], 'queries'))
        #save_query_path = os.path.join("/media/s/新加卷/voxkitti/dataset/sequences_msnet3d", img_metas[0]['sequence_id'], 'queries', frame_id + ".query_a_mIoU_5292_pre_7726")
...
class BFFM(nn.Module):
    def __init__(self, in_channels_x, in_channels_y, mid_channels, after_relu=False, with_channel=False,
                 BatchNorm=nn.BatchNorm2d):
        super(PagFM, self).__init__()
        self.with_channel = with_channel
        self.after_relu = after_relu
        self.f_x = nn.Sequential(
            nn.Conv2d(in_channels_x, mid_channels, kernel_size=1, bias=False),
            BatchNorm(mid_channels)
        )
        self.f_y = nn.Sequential(
            nn.Conv2d(in_channels_y, mid_channels, kernel_size=1, bias=False),
            BatchNorm(mid_channels)
        )
        if with_channel:

            self.up = nn.Sequential(
                nn.Conv2d(mid_channels, in_channels_x, kernel_size=1, bias=False),
                BatchNorm(in_channels_x)
            )
        if after_relu:
            self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x, y):
        ...
        output = torch.cat((x * (1 - sim_map), y * sim_map), dim=1)
